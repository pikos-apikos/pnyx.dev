...
def nav(page: Page) -> Node:
    ...
    links = (
        ...
        element("a", "Ημερολόγιο" if el_lang else "Journal", href="/el/journal/generated-menu/" if el_lang else "/journal/generated-menu/"),
...
def header(page: Page) -> Node:
    ...
    return element("header",
        element("a", element("span", "Π", class_="brand-mark", aria_hidden="true"), element("span", "Pnyx"), class_="brand", href="/el/" if el_lang else "/", aria_label="Αρχική σελίδα Pnyx" if el_lang else "Pnyx home"),
        nav(page), class_="site-header")


def footer(page: Page) -> Node:
    el_lang = page.lang == "el"
    if page.journal:
        return element("footer",
            element("p", Raw("<strong>Ημερολόγιο Pnyx</strong> — το ιστορικό που δεν χωρά στο diff." if el_lang else "<strong>Pnyx journal</strong> — history that does not fit in the diff.")),
            element("p", element("a", "Επιστροφή στην Pnyx" if el_lang else "Return to Pnyx", href="/el/" if el_lang else "/")))
    return element("footer",
        element("p", Raw("<strong>Pnyx</strong> — δημόσια σκέψη, ανθρώπινη κρίση, ελέγξιμη μνήμη." if el_lang else "<strong>Pnyx</strong> — public reasoning, human judgment, auditable memory.")),
        element("p", element("a", "Πηγαίος κώδικας ιστοτόπου" if el_lang else "Website source", href="https://github.com/pikos-apikos/pnyx.dev")))
...
def compile_page(page: Page) -> str:
    ...
    body = element("body", element("a", skip, class_="skip-link", href="#main"), header(page), Raw(content), footer(page))
    document = element("html", head(page), body, lang=page.lang)
    return "<!doctype html>\n<!-- Generated by site/build.py; edit site/content or compiler components. -->\n" + document.render() + "\n"
